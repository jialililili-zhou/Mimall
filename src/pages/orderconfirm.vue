        ...
        <div class="order-box">
          ...
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <!-- 2.1地址信息 -->
              <!-- 谁选中就把谁的index值赋给checkIndex 每一个元素在把自己的index与checkIndex相比较，相同则添加checked类名-->
    ...
    <modal title='新增确认' btnType='1' :showModal="showEditModal"
      ...
      <template v-slot:body>
        <div class="edit-wrapper">
          ...
          <div class="item">
            <input type="text" class="input" placeholder="邮政编码" v-model="checkedItem.receiverZip">
          </div>
        </div>
      </template>
    </modal>
...
export default {
  ...
  data(){
    return{
      ...
      cartTotalPrice:0,   //购物车中已选中商品总金额
      count:0,    //商品结算数量
      checkedItem:{},     //选中的商品对象
      userAction:'',    //用户行为 0：新增，1：编辑，2：删除
      showDelModal:false,     //是否显示删除弹框
      showEditModal:false,    //是否显示编辑框,同时控制编辑和新增地址框的显示
      checkIndex: 0,      //选中的收获地址索引
    }
  },
  mounted(){
    this.getAddressList(),
  ...
  methods:{
    ...
    getCartList(){
      this.axios.get('/carts').then(res=>{
        ...
        this.cartList.map(item=>{
          this.count += item.quantity     //quantity每件商品的数量
        })
      })
    },
    // --------------以下三个函数与modal框的@cancel绑定
    // 删除地址
    delAddress(item){
    ...
    // ------------------与modal框的@submit绑定,负责新增、编辑、删除地址数据的提交
    submitAddress(){
      let {checkedItem,userAction} = this     //ES6对象结构
      ...
      if(userAction == '0' || userAction == '1'){
        ...
        if(!receiverName){
          ...
          return
        }else if(!receiverMobile || !/\d{11}/.test(receiverMobile)){        //正则匹配，\d表示数字开头，{11}表示11位
      ...
      this.axios[method](url,params).then(()=>{        //动态获取axios的方法
        this.closeModal()
        this.getAddressList()       //删除地址后，重新从服务端获取数据渲染，防止多个平台登录，地址不一样的情况
        this.$message.success('操作成功')
      })         
    },
    // 确定删除成功后，把对象值、用户行为赋空，弹框消失
    closeModal(){
      this.checkedItem = {}
      this.userAction = ''
    ...
    orderSubmit(){
      ...
      if(!item){
        ...
        this.axios.post('/orders',{
          shippingId: item.id        //地址的id号
        }).then(res=>{
          this.$router.push({
            path:'/order/pay',
            query:{
              orderNo: res.orderNo      //返回的结果中包含生成的订单号
            }
          })
        })
      }
